...
# Networking Module - No NAT Gateway for dev
module "networking" {
  ...
  enable_nat_gateway    = var.enable_nat_gateway  # Set to false for dev
}
...
# ECS Module - Using public subnets with public IP
module "ecs" {
  source = "../../modules/ecs"
  ...
  environment               = var.environment
  vpc_id                    = module.networking.vpc_id
  subnet_ids                = module.networking.public_subnet_ids  # Use public subnets
  allowed_security_group_ids = [module.alb.alb_security_group_id]
  
  cpu           = var.n8n_cpu
  memory        = var.n8n_memory
  desired_count = var.n8n_desired_count
  assign_public_ip = true  # Enable public IP for tasks in public subnets
...
# RDS Module - Using public subnets for dev (less secure but cost-effective)
module "database" {
  source = "../../modules/rds"
